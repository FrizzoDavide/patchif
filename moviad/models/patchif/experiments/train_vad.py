...
from moviad.datasets.mvtec.mvtec_dataset import CATEGORIES, MVTecDataset
from moviad.datasets.realiad.realiad_dataset import RealIadClassEnum, RealIadDataset
...
from moviad.utilities.exp_configurations import (
    AD_LAYERS,
    DATASET_NAMES,
    DATASET_PATHS,
    MODEL_NAMES,
...
parser.add_argument(
    "--category", type=str, default="pill", help="Dataset category to test"
)
parser.add_argument(
    "--ad_model_type", type=str, default="eif", help="Type of AD model, eif or if"
)
parser.add_argument(
    "--backbone", type=str, default="mobilenet_v2", help="Model backbone"
)
parser.add_argument(
    "--device_num", type=int, default=0, help="Number of the CUDA device to use"
)
parser.add_argument(
    "--n_estimators",
    type=int,
...
parser.add_argument(
    "--use_saved_model",
    action="store_true",
    help="Flag to load the state dict of a saved model",
)
parser.add_argument(
    "--train_model",
    action="store_true",
    help="Flag to train the model, otherwise load the state dict of an already saved model",
)
parser.add_argument(
    "--anomaly_map", action="store_true", help="Flag to produce the anomaly maps"
)

args = parser.parse_args()

setproctitle.setproctitle(
    f"exp_{args.dataset_name}_{args.category}_{args.model_name}_{args.backbone}"
)

assert (
    args.dataset_name in DATASET_NAMES
), f"Dataset {args.dataset_name} not supported. Supported datasets: {DATASET_NAMES}"
assert (
    args.category in CATEGORIES
), f"Dataset {args.dataset_name} not supported. Supported datasets: {CATEGORIES}"
assert (
    args.model_name in MODEL_NAMES
), f"Model {args.model_name} not supported. Supported models: {MODEL_NAMES}"
assert (
    args.backbone in TORCH_BACKBONES or args.backbone in OTHERS_BACKBONES
), f"Backbone {args.backbone} not supported. Supported backbones: {TORCH_BACKBONES + OTHERS_BACKBONES}"
...
print("#" * 50)
print(
    f"Train dataset contamination ratio: {train_dataset.compute_contamination_ratio()}"
)
print("#" * 50)
...
if args.use_saved_model:
    ...
    try:
        ...
        print(
            "Model directory not found. Please train the model first or check the path."
        )
