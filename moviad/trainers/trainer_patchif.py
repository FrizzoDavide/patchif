...
class TrainerPatchIF(Trainer):
    def __init__(
        ...
        """
        super().__init__(model, train_dataloader, test_dataloader, device, logger)
    ...
    def train(self):
        ...
        if self.model.subsample_ratio == 1.0:
            ...
            print("#" * 50)
            subsample_size = int(
                embedding_vectors.shape[0] * (1 - self.model.subsample_ratio)
            )
            random_rows = torch.tensor(
                sample(range(embedding_vectors.shape[0]), subsample_size)
            )
