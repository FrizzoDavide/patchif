...
from moviad.datasets.mvtec.mvtec_dataset import (
    MVTecDataset,
    CATEGORIES,
    load_train_test_data,
    load_contaminate_train_test_data,
)
...
from moviad.utilities.manage_files import (
    get_current_time,
    save_element,
    generate_path,
...
def main(args):
    assert [
        category in CATEGORIES for category in args.categories
    ], f"Some categories are not valid. Available categories: {CATEGORIES}"
    ...
    patchif_results_path = generate_path(
        basepath=os.getcwd(), folders=["patchif_results"]
    )
    ...
    for seed in seeds:
        ...
        print("#" * 50)
        print(f"Experiment for seed: {seed}") if not args.inference else print(
            f"Experiment for seed: {args.best_seed}"
        )
        print("#" * 50)
        ...
        for category in args.categories:
            ...
            run_name = (
                f"{exp_name}_{category}_seed_{seed}"
                if not args.inference
                else f"{exp_name}_{category}_seed_{seed}_inference"
            )
            ...
            if args.train:
                ...
                print(
                    f"Contamination ratio: {train_dataset.compute_contamination_ratio()}"
                )
                print("#" * 50)
                ...
                print(
                    f"Contamination ratio: {test_dataset.compute_contamination_ratio()}"
                )
                print("#" * 50)
                ...
                if args.save_model:
                    ...
                    print("#" * 50)
                    print(
                        f"Model state dict successfully saved in: {model_dirpath_seed}"
                    )
                    print("#" * 50)
            ...
            if args.test:
                ...
                model_path = get_most_recent_file(
                    model_dirpath_seed, file_pos=args.file_pos
                )
                ...
                if args.save_figures:
                    ...
                    for images, labels, anomaly_labels, masks, paths in tqdm(
                        iter(test_loader)
                    ):
                        anomaly_maps, pred_scores = model(images.to(device))
                        anomaly_maps = torch.permute(
                            torch.tensor(anomaly_maps), (0, 2, 3, 1)
                        )

                        for i in range(anomaly_maps.shape[0]):
                            print("#" * 50)
                            print(
                                f"Creating anomaly map for image with path: {paths[i]}"
                            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_memory_bank", action="store_true", help="Save the memory bank"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="mvtec",
        help="Dataset name, available: mvtec",
    )
    parser.add_argument(
        "--n_runs", type=int, default=1, help="Number of runs for the experiment"
    )
    parser.add_argument(
        "--best_seed",
        type=int,
        default=0,
        help="Seed to use for the inference experiments",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="file position in a folder, to load the last saved model for testing",
    )
    # model parameters
    parser.add_argument(
        "--backbone_model_name",
        type=str,
        help="Available backbones: resnet18, wide_resnet50_2, mobilenet_v2, mcunet-in3",
    )
    parser.add_argument(
        "--ad_model_name", type=str, default="eif", help="Type of AD model, eif or if"
    )
    parser.add_argument(
        "--n_estimators",
        type=int,
        default=100,
        help="Number of estimators for the IF/EIF model, for patchif",
    )
    parser.add_argument(
        "--max_nodes",
        type=int,
        default=10000,
        help="Maximum number of nodes per tree in the IF/EIF model, for patchif",
    )
    parser.add_argument(
        "--plus", action="store_true", help="Flag to use the EIF or EIF+ AD model"
    )
    parser.add_argument(
        "--subsample_ratio",
        type=float,
        default=1.0,
        help="Subsample ratio for the memory bank",
    )
    # dataset parameters
    ...
    parser.add_argument(
        "--contaminate",
        action="store_true",
        help="Contaminate the dataset with anomalies",
    )
    parser.add_argument(
        "--contamination_ratio",
        type=float,
        default=0.1,
        help="Contamination ratio for the dataset",
    )
